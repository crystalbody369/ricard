...
def sample():
    ...
    # 実アプリ(PAGE)の <style> をそのまま流用し、本物そっくりの見た目にする。
    css = PAGE.split("<style>", 1)[1].split("</style>", 1)[0]
    html = ('<!doctype html><html lang="ja"><head><meta charset="utf-8">'
            '<meta name="viewport" content="width=device-width, initial-scale=1, maximum-scale=1">'
            '<title>サンプル・理カード</title><style>' + css + '</style></head>'
            '<body><div class="wrap">' + SAMPLE_INNER + '</div></body></html>')
...
<p class="note" style="text-align:center">お問い合わせ・特定商取引法に基づく表記は準備中です。</p>"""


# ログイン後の相談画面のサンプル（実アプリと同じCSSで本物そっくりに見せる）。
# 実アプリのクラス（.card / textarea / button / .detail / .exchip 等）をそのまま使う。
SAMPLE_INNER = """<h1>理カード</h1><p class="tag">ログイン後の画面イメージ</p>
<div class="banner" style="display:block">🔎 これは<b>サンプル画面</b>です。ログイン後は、この画面で実際に相談できます。ご利用には登録が必要です。</div>

<div class="card">
  <h2>理に相談する</h2>
  <p class="note" style="text-align:left; margin:0 0 10px">気になった出来事を書くと、理の視点で静かに観ます。当てるのではなく、整えるために。</p>
  <div id="cexamples" style="margin:0 0 10px">
    <p class="note" style="text-align:left;margin:0 0 6px">はじめての方へ — どう書けばいい？ 見本を見る（無料・相談は消費しません）：</p>
    <div style="display:flex;flex-wrap:wrap;gap:6px">
      <span class="exchip">🛑 直前にトラブルで止まった</span>
      <span class="exchip">🤝 物の不調と縁が重なった</span>
      <span class="exchip">🔁 小さなことが続いた</span>
    </div>
  </div>
  <textarea rows="3" readonly style="background:#fff">大事なメールを送ろうとした直前に、パソコンが急に固まって動かなくなりました。最初は腹が立ちましたが、再起動してもう一度メールを見直したところ、宛先を一人間違えていたことに気づきました。もしそのまま送っていたら、かなり失礼なことになっていたと思います。</textarea>
  <div class="ccount"><span>125</span>/500　<span id="cremain">残り3回（無料お試し）</span></div>
  <label style="margin-top:10px">今の気持ち・状況・取り組んでいること（任意）</label>
  <textarea rows="2" readonly style="background:#fff">最近、仕事を早く進めようとして少し焦っています。細かい確認を後回しにして、とにかく先に送ってしまおうという気持ちがありました。</textarea>
  <p class="note" style="text-align:left;margin:4px 0 0">※気持ちや状況も書くほど、あなたに合った観方になります。</p>
  <button style="opacity:.6;cursor:default">理に観てもらう（サンプル）</button>
  <p class="note" style="text-align:left">※入力した文章はAI（Claude）に送られ、回答を作ります。文章は保存しません。</p>
  <div class="detail" style="display:block; white-space:pre-wrap; line-height:1.95; font-size:14.5px;">焦って「先に送ろう」とする気持ちが、確かめる目を薄くしていた。そこへ強制的に止められ、もう一度見る間ができた――そんな流れに見えます。一歩早く送るより、一手確かめてから送るほうが、結局は早いことがあります。送る前に三秒だけ止まる、その小さな間を意識してみては。意味は流してもよいですが、焦りだけはそっと受け取っておいてください。</div>
</div>

<div class="card">
  <h2>料金</h2>
  <p class="note" style="text-align:left">無料でお試し → 続けたい方は <b>相談30回ぶん ¥500</b>（買い切り・自動更新なし）。</p>
</div>

<div class="card">
  <h2>実際に使うには</h2>
  <p class="note" style="text-align:left">現在は<b>招待制</b>です。紹介コードをお持ちの方はご登録ください。</p>
  <button onclick="location.href='/login'">ログイン</button>
  <button class="ghost" onclick="location.href='/register'">紹介コードで登録</button>
</div>

<p class="foot"><a href="/" style="color:var(--gold)">← トップ（サービス説明）に戻る</a></p>"""
