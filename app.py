...
@app.route("/legal")
def legal():
    # 特定商取引法に基づく表記（ログイン有無を問わず誰でも閲覧可）。
    body = ('<h1>Kizuki</h1><p class="tag">特定商取引法に基づく表記</p>'
            + LEGAL_CARD + '<p class="lnk"><a href="/">戻る</a></p>')
...
# 特定商取引法に基づく表記（公開LPとログイン後 /legal で共用）。
LEGAL_CARD = """<div class="card"><h2>特定商取引法に基づく表記</h2>
<table>
