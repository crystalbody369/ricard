...
PACK_NAME = "気づき 相談%d回パック" % PACK_CREDITS
...
var I18N = {
  ja: {h1:'気づき', tag:'今日を、当てずに整える。', h2today:'今日の理', gear:'⚙ 設定',
...
function copyInvite(){
  ...
  if(navigator.share){
    navigator.share({ title:'気づき', text:'二人の縁、見てみない？', url: link }).catch(function(){});
    return;
  }
  // PC等：クリップボードにコピー
...
SETUP_BODY = """<h1>気づき</h1><p class="tag">最初の管理者を作成します</p>
...
LANDING_BODY = """<h1>気づき</h1><p class="tag">今日を、当てずに整える。</p>

<div class="card"><h2>気づきとは</h2>
...
LOGIN_BODY = """<h1>気づき</h1><p class="tag">今日を、当てずに整える。</p>
...
REGISTER_BODY = """<h1>気づき</h1><p class="tag">紹介コードで登録</p>
...
FORGOT_BODY = """<h1>気づき</h1><p class="tag">パスワードの再設定</p>
...
RESET_BODY = """<h1>気づき</h1><p class="tag">新しいパスワードの設定</p>
...
def forgot():
    ...
    if request.method == "POST":
        ...
        if res and mailer.mail_configured():
            ...
            body = ("気づき：パスワード再設定のご案内\n\n"
                    "下のリンクから新しいパスワードを設定してください（60分間有効）。\n"
                    + link + "\n\n"
                    "※このメールに心当たりがない場合は、破棄してください。\n"
                    "気づき（栄宏ライフ株式会社）")
            mailer.send_email(email, "気づき パスワード再設定", body)
...
def reset():
    ...
    if request.method == "POST":
        ...
        if uname:
            return _shell("完了", '<h1>気づき</h1><p class="tag">設定が完了しました</p>'
...
def admin():
    ...
    body = """<h1>管理者画面</h1><p class="tag">気づき・オーナー専用（{user}）</p>
{msg}
