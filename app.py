# -*- coding: utf-8 -*-
"""Kizuki — 誰でも自分のカードを作れるモバイルWeb（＋縁モード）。
...
PACK_NAME = "Kizuki 相談%d回パック" % PACK_CREDITS
...
var CEX = {
  ...
  en: [
    {chip:'🛑 Stopped right before sending',
     ...
     ans:'The urge to "just send it first" had dulled your eye for checking. Right then you were forcibly stopped, and a moment to look again opened up — that is how the flow seems. Rather than sending a step early, checking one move first is often faster in the end. Try to keep that small pause, just three seconds before sending. You may let the meaning go, but do gently take in the haste itself.'},
...
@app.route("/legal")
def legal():
    # 特定商取引法に基づく表記（誰でも閲覧可・選択中の言語に追従）。
    data_json = json.dumps(LEGAL_DATA, ensure_ascii=False)
    body = ('<h1 translate="no">Kizuki</h1><p class="tag" id="legaltag"></p>'
            '<div class="card"><h2 id="legalh2"></h2><table id="legaltable"></table></div>'
            '<p class="lnk"><a href="/" id="legalback">戻る</a></p>'
            '<script>(function(){var D=' + data_json + ';'
            'var l="ja";try{l=localStorage.getItem("ricard_lang")||"ja";}catch(e){}'
            'if(!D[l])l="ja";var d=D[l];'
            'document.getElementById("legalh2").textContent=d.heading;'
            'document.getElementById("legaltag").textContent=d.heading;'
            'document.getElementById("legalback").textContent=d.back;'
            'var h="";for(var i=0;i<d.rows.length;i++){'
            'h+="<tr><td style=\\"color:var(--sub);white-space:nowrap;vertical-align:top\\">"'
            '+d.rows[i][0]+"</td><td>"+d.rows[i][1]+"</td></tr>";}'
            'document.getElementById("legaltable").innerHTML=h;'
            'document.documentElement.lang=({zh:"zh-Hant",cn:"zh-Hans",en:"en"}[l]||"ja");'
            '})();</script>')
    return _shell("特定商取引法に基づく表記", body)
...
SETUP_BODY = """<h1>Kizuki</h1><p class="tag">最初の管理者を作成します</p>
...
# 特定商取引法に基づく表記（公開LPは日本語、/legal は選択言語に追従）。
_EMAIL_HTML = '<a href="mailto:ahiro@ahiro.page">ahiro@ahiro.page</a>'
_COMPANY = "栄宏ライフ株式会社（Ahiro Life Co., Ltd.）"
LEGAL_DATA = {
    "ja": {"heading": "特定商取引法に基づく表記", "back": "戻る", "rows": [
        ["販売事業者", _COMPANY],
        ["運営統括責任者", "請求があれば遅滞なく開示します"],
        ["所在地", "請求があれば遅滞なく開示します"],
        ["電話番号", "請求があれば遅滞なく開示します（お問い合わせはメールでお願いします）"],
        ["お問い合わせ", _EMAIL_HTML],
        ["販売価格", "相談30回パック ¥500（税込）"],
        ["商品代金以外の費用", "なし（通信にかかる費用はお客様のご負担となります）"],
        ["支払方法", "クレジットカード（Stripe）"],
        ["支払時期", "購入手続きの完了時にお支払いが確定します"],
        ["提供時期", "決済完了後ただちに、アプリ内へ相談回数を付与します"],
        ["返品・キャンセル", "デジタルサービスの性質上、付与後の返金・キャンセルはお受けできません。不具合等はお問い合わせください。"],
    ]},
    "zh": {"heading": "特定商取引法（日本）資訊揭露", "back": "返回", "rows": [
        ["銷售事業者", _COMPANY],
        ["營運負責人", "應要求將立即揭露"],
        ["所在地", "應要求將立即揭露"],
        ["電話號碼", "應要求將立即揭露（請以電子郵件聯絡）"],
        ["聯絡方式", _EMAIL_HTML],
        ["銷售價格", "諮詢30次方案 ¥500（含稅）"],
        ["商品價金以外的費用", "無（通訊費用由客戶負擔）"],
        ["付款方式", "信用卡（Stripe）"],
        ["付款時間", "於購買手續完成時確定付款"],
        ["提供時間", "付款完成後立即於App內發放諮詢次數"],
        ["退款・取消", "因屬數位服務，發放後恕不接受退款或取消。如有問題請與我們聯絡。"],
    ]},
    "cn": {"heading": "特定商取引法（日本）信息披露", "back": "返回", "rows": [
        ["销售事业者", _COMPANY],
        ["运营负责人", "应要求将立即披露"],
        ["所在地", "应要求将立即披露"],
        ["电话号码", "应要求将立即披露（请以电子邮件联系）"],
        ["联系方式", _EMAIL_HTML],
        ["销售价格", "咨询30次套餐 ¥500（含税）"],
        ["商品价款以外的费用", "无（通讯费用由客户承担）"],
        ["付款方式", "信用卡（Stripe）"],
        ["付款时间", "于购买手续完成时确定付款"],
        ["提供时间", "付款完成后立即于App内发放咨询次数"],
        ["退款・取消", "因属数字服务，发放后恕不接受退款或取消。如有问题请与我们联系。"],
    ]},
    "en": {"heading": "Legal Notice (Japan – Specified Commercial Transactions Act)", "back": "Back", "rows": [
        ["Seller", _COMPANY],
        ["Operations Manager", "Disclosed without delay upon request"],
        ["Address", "Disclosed without delay upon request"],
        ["Phone", "Disclosed upon request (please contact us by email)"],
        ["Contact", _EMAIL_HTML],
        ["Price", "30-consultation pack ¥500 (tax incl.)"],
        ["Additional fees", "None (data/communication charges are borne by the customer)"],
        ["Payment method", "Credit card (Stripe)"],
        ["Payment timing", "Payment is confirmed upon completion of checkout"],
        ["Delivery", "Consultation credits are granted in-app immediately after payment"],
        ["Returns / Cancellation", "Due to the nature of digital services, refunds or cancellations are not accepted after credits are granted. Please contact us with any issues."],
    ]},
}


def _legal_rows_html(rows):
    return "".join(
        '<tr><td style="color:var(--sub);white-space:nowrap;vertical-align:top">%s</td><td>%s</td></tr>' % (l, v)
        for l, v in rows)


# 公開LP用（日本語固定）。
LEGAL_CARD = '<div class="card"><h2>%s</h2><table>%s</table></div>' % (
    LEGAL_DATA["ja"]["heading"], _legal_rows_html(LEGAL_DATA["ja"]["rows"]))
...
LANDING_L = {
    ...
    "cn": {"tag": "不为了算准，而是整理今天。",
           ...
           "ev": "正要寄出一封重要邮件前，电脑突然死机。重启后再看一次，才发现有一个收件人寄错了。",
    ...
    "en": {"tag": "Settle today, not predict it.",
           "whatH": "About Kizuki",
           "whatP": "Write down what's on your mind, and we observe it quietly through 'Ri' — an app for reflection, not fortune-telling. Rather than labeling events good or bad, we look together at what to notice and what to set in order, right now.",
           ...
           "ans": "The urge to \"just send it first\" had dulled your eye for checking. Right then you were forcibly stopped, and a moment to look again opened up — that's how the flow seems. Rather than sending a step early, checking one move first is often faster in the end. Try to keep that small pause, just three seconds before sending. You may let the meaning go, but do gently take in the haste itself.",
           "priceH": "Pricing & what you get", "priceLead": "Try it free first. To keep using it:", "priceBig": "30 consultations = ¥500 (tax incl.)",
...
LOGIN_BODY = """<h1>Kizuki</h1><p class="tag">今日を、当てずに整える。</p>
...
REGISTER_BODY = """<h1>Kizuki</h1><p class="tag">紹介コードで登録</p>
...
FORGOT_BODY = """<h1>Kizuki</h1><p class="tag">パスワードの再設定</p>
...
RESET_BODY = """<h1>Kizuki</h1><p class="tag">新しいパスワードの設定</p>
...
def forgot():
    ...
    if request.method == "POST":
        ...
        if res and mailer.mail_configured():
            ...
            body = ("Kizuki：パスワード再設定のご案内\n\n"
                    "下のリンクから新しいパスワードを設定してください（60分間有効）。\n"
                    + link + "\n\n"
                    "※このメールに心当たりがない場合は、破棄してください。\n"
                    "Kizuki（栄宏ライフ株式会社）")
            mailer.send_email(email, "Kizuki パスワード再設定", body)
...
def reset():
    ...
    if request.method == "POST":
        ...
        if uname:
            return _shell("完了", '<h1>Kizuki</h1><p class="tag">設定が完了しました</p>'
...
def admin():
    ...
    if request.method == "POST":
        ...
        try:
            if a == "code_create":
                ...
                msg = "%d回分のクレジットを付与しました。" % PACK_CREDITS
            elif a == "user_resetpw":
    ...
    body = """<h1>管理者画面</h1><p class="tag">Kizuki・オーナー専用（{user}）</p>
{msg}
